...
describe("GET /file/:file_id/download", () => {
  const file_id = 123;
  const file_name = "Copy of CTTV - Thực đơn tuần 4.T7.xlsx";

  it("should return an Excel file for valid file_id", async () => {
    const db = getDB();
    ...
    const res = await request(app).get(`meal-menus/file/${file_id}/download`);

    expect(res.statusCode).toBe(200);
    expect(res.header["content-type"]).toBe(
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    );
    expect(res.header["content-disposition"]).toContain(file_name);
